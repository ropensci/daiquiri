...
stratify <- !is.null(params$aggregated_data$aggregated_fields_stratified)
agg_fields_strat <- params$aggregated_data$aggregated_fields_stratified
stratum_names <- params$aggregated_data$stratified_by_values
...
if (!stratify) {
  ...
  for (i in seq_along(stratum_names)) {
    cat("\n####", stratum_names[i], "\n")
    p <-
      plot_overview_combo_static(
...
# TODO: Problem with doing this is that non-combo plots become too tall. Need to stratify duplicates and allfields too.
# Also need to think about what to have in stratify_by field's tab.
# (can't modify chunk options within a chunk to get different fig heights (https://github.com/yihui/knitr/issues/841))
individual_fields_fig_height <- knitr::opts_chunk$get("fig.height")

if (stratify) {
  individual_fields_fig_height <- 1 + length(params$aggregated_data$stratified_by_values)*0.8
}
```
...
for (i in seq_along(names(agg_fields))) {
  ...
  for (agg_fun in agg_funs) {
    if (startsWith(agg_fun, "subcat")) {
      ...
      if (stratify && agg_fun %in% names(agg_fields_strat[[field_name]]$values)) {
