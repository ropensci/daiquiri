...
#' walkthrough vignette: \href{../doc/walkthrough.html}{\code{vignette("walkthrough", package = "daiquiri")}}.
#'
#'
#' @docType package
#' @name daiquiri
...
#'
#' @param df A data frame. Rectangular data can be read from file using \code{\link{read_data}}. See details.
...
#' Operates as a restricted implementation of \code{readr::read_delim}.
#'
#' @param file A string containing path of file containing data to load,
#' or a URL starting http://, file://, etc. Compressed files with extension ‘.gz’, ‘.bz2’, ".xz" and ".zip" are supported.
#' @param delim Single character used to separate fields within a record. E.g. \code{","} or \code{"\t"}
#' @param col_names Either TRUE, FALSE or a character vector of column names. If TRUE, the first row of the input will be used as the column names, and will not be included in the data frame. If FALSE, column names will be generated automatically: X1, X2, X3 etc. Default = TRUE
#' @param quote Single character used to quote strings.
#' @param trim_ws Should leading and trailing whitespace be trimmed from each field?
#' @param comment A string used to identify comments. Any text after the comment characters will be silently ignored
#' @param skip Number of lines to skip before reading data. If \code{comment} is supplied any commented lines are ignored after skipping
...
read_data <- function(file, delim = NULL, col_names = TRUE, quote = "\"", trim_ws = TRUE,
	...
	readr::read_delim(file, delim = delim, quote = quote, col_names = col_names, col_types = readr::cols(.default = "c"),
										...
										skip = skip, n_max = n_max, name_repair = "unique",
										progress = showprogress, skip_empty_rows = TRUE, lazy = TRUE)
