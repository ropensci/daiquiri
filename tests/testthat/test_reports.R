...
test_that("report_data() params are present and of correct type", {
	...
	expect_error(report_data(sourcedata = structure(list(datafields = NA), class = "sourcedata"),
...
test_that("report_data() creates report and returns path successfully", {
	...
	testreportpath <- report_data(testsourcedata, testaggregatedata, save_directory = tempdir(), save_filename = "daiquiri_testthatreport", showprogress = FALSE)
