...
The `example_data` dataset is provided with this package as a csv file, and contains (synthetic) examples of antibiotic prescriptions given in a hospital over a period of a year, with one row per prescription. It contains 8 columns:
...
filename <- system.file("extdata", "example_data.csv", package = "daiquiri")
...
Here is a list of the possible field types for the columns. Different time series will be generated depending on the type of field. All types will have time series generated for the number values present, as well as the number and percentage of missing values.
...
For the `example_data` dataset above, we will use the PrescriptionDate as the timepoint field, and specify the other columns as follows:
...
For the `example_data` dataset above, we will set the aggregation granularity to `day`, and save a report in the current directory.
...
<img src="../man/figures/example_data_source_fieldsimported.png" width="500" />
...
<img src="../man/figures/example_data_aggregated_valuespresent.png" width="500" />
...
<img src="../man/figures/example_data_allfields_missing_perc.png" width="500" />
...
examplesourcedata <- prepare_data(
  example_data, 
  fieldtypes = fts, 
  na=c("","NULL")
)
...
exampleaggregate_byday <- aggregate_data(
  examplesourcedata,
  aggregation_timeunit = "day"
)

# aggregate the same sourcedata object by a different granularity
exampleaggregate_byweek <- aggregate_data(
  examplesourcedata,
...
report_data(
  ...
  aggregatedata = exampleaggregate_byday,
  save_directory = ".",
  save_filename = "example_data_byday"
)

report_data(
  sourcedata = examplesourcedata,
  aggregatedata = exampleaggregate_byweek,
  save_directory = ".",
  save_filename = "example_data_byweek"
)
