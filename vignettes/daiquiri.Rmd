...
TIP: If you have a lot of columns in your dataset you can use the `fieldtypes_template()` function to print a template specification to the console that you can copy and edit.
...
NOTE ABOUT COLUMN NAMES: You can choose to override the existing column names in the dataframe with those in the `fieldtypes()` specification by setting the `override_columnnames` parameter to `TRUE`, in which case they must be specified in the correct order. If `override_columnnames = FALSE` (the default) then the names in the `fieldtypes()` specification must match exactly to those in the dataframe.
